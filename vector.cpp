		   ...
		   std::function<bool(ELEM_TYPE, ELEM_TYPE)> op)
{
  ...
  for (unsigned ix = start; ix < elems; ++ix)
    {
      if (vecRegs_.skipMaskAgnosticMask(vd, masked, ix))
	continue;
	  ...
	  if (elemSize == 1)
	    {
	      uint8_t x = 0;
	      memRead(pa1, pa2, x);
	      vecRegs_.write(vd, ix, groupX8, x);
	    }
	  else if (elemSize == 2)
	    {
	      uint16_t x = 0;
      ...
      for (unsigned field = 0; field < fieldCount; ++field, faddr += elemSize)
	{
	  ...
	  if (cause == ExceptionCause::NONE)
	      ...
	      if (elemSize == 1)
		{
		  uint8_t x = 0;
		  memRead(pa1, pa2, x);
		  vecRegs_.write(dvg, ix, groupX8, x);
		}
	      else if (elemSize == 2)
		{
		  uint16_t x = 0;
		     ...
		     unsigned start, unsigned elems, bool masked)
{
  ...
  for (unsigned ix = start; ix < elems; ++ix)
    {
      if (vecRegs_.skipMaskAgnostic(vd, group2x, masked, ix))
	continue;
