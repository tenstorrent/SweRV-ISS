...
struct Args
{
  ...
  std::string initStateFile;             // Output: initial state of memory lines used in run.
...
/// Return true on success and false on failure. Exit program
/// if --help is used.
static
bool
...
determineIsa(const HartConfig& config, const Args& args, bool clib, std::string& isa)
{
  ...
  if (isa.empty() and not args.raw)
    {
      if (args.verbose)
	std::cerr << "No ISA specified: Defaulting to imacfd\n";
      isa = "imacfd";
    }
...
/// Determine register width (xlen) from ELF file.  Return true if
