...
class whisper(pluginTemplate):
    __model__ = "whisper"

    #TODO: It'd be nice to figure out a way to share this number between the executable and this file.
    __version__ = "1.800"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","whisper")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      for ext in ["I", "M", "F", "D", "C"]:
          if ext in ispec["ISA"]:
            self.isa += ext.lower()
      for ext in ["Zba", "Zbb", "Zbc", "Zbkb", "Zbkx", "Zbs", "Zknd", "Zkne", "Zknh", "Zksed", "Zksh"]:
          if ext in ispec["ISA"]:
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --isa={0} --testsignature={1} --target={2}'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
